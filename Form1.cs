        ...
        private void dds_save(object sender, EventArgs e)
        {
            ...
            using (FileStream sf = new FileStream(fileName, FileMode.Open, FileAccess.Read))
            {
                ...
                using (FileStream ddsFile = new FileStream("temp.dds", FileMode.Create, FileAccess.Write))
                {
                    ...
                    ddsFile.Write(h_flg, 0, h_flg.Length);
                    ddsFile.Write(BitConverter.GetBytes(x), 0, 4); // Height
                    ddsFile.Write(BitConverter.GetBytes(y), 0, 4); // Width
                    ddsFile.Write(BitConverter.GetBytes(0), 0, 4); // Linear size
                    ddsFile.Write(BitConverter.GetBytes(0), 0, 4); // Depth
                    ddsFile.Write(BitConverter.GetBytes(0), 0, 4); // Mipmap count
                    ddsFile.Write(BitConverter.GetBytes(0), 0, 44); // Reserved
                    ddsFile.Write(flags, 0, flags.Length); // Flags
                    ddsFile.Write(cdc, 0, cdc.Length); // FourCC
                    ddsFile.Write(bpp, 0, bpp.Length); // BPP
                    ddsFile.Write(rgbaMask, 0, rgbaMask.Length);
                    ddsFile.Write(BitConverter.GetBytes(0x08), 0, 4); // Caps
                    ddsFile.Write(BitConverter.GetBytes(0x1040), 0, 4); // Caps2
                    ddsFile.Write(BitConverter.GetBytes(0), 0, 16); // Reserved
